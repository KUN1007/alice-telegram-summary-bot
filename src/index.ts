...
export default {
  async scheduled(
    ...
    const cacheKey = new Request(
      `https://dummy-url/${env.SECRET_TELEGRAM_API_TOKEN}`
    )
    ...
    for (const [id, group] of groups.entries()) {
      ...
      console.debug(
        `Processing group ${id + 1}/${groups.length}: ${group.groupId}`
      )
      ...
      const res = await fetch(
        `https://api.telegram.org/bot${env.SECRET_TELEGRAM_API_TOKEN}/sendMessage`,
        {
          ...
          body: JSON.stringify({
            ...
            text: processMarkdownLinks(
              telegramifyMarkdown(
                messageTemplate(result.response.text()),
                'keep'
              )
            ),
  ...
  .map(
    (r: any) =>
      `${r.userName}: ${r.content} ${
        r.messageId == null
          ? ''
          : `[link](https://t.me/c/${parseInt(r.groupId.slice(2))}/${
              r.messageId
            })`
      }`
  )
...
[关键字2](链接本体), 在链接的两侧加空格`,
        ...
        if (result.response.promptFeedback?.blockReason) {
          response_text =
            '无法回答, 理由' + result.response.promptFeedback.blockReason
        } else {
          response_text = processMarkdownLinks(
            telegramifyMarkdown(result.response.text(), 'keep')
          )
        }
      ...
      .on('summary', async (bot) => {
        ...
        if (bot.update.message!.text!.split(' ').length === 1) {
          await bot.reply(
            '请输入要查询的时间范围/消息数量, 如 /summary 114h 或 /summary 514'
          )
          return new Response('ok')
        }
